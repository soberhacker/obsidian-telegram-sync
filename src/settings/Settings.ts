...
export class TelegramSyncSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    const botFatherSetting = new Setting(containerEl)
      .setName('Bot Token')
      .setDesc("Enter your Telegram bot token. To create a new bot click on -> ") //@botFather added down
      .addText((text) =>
        text
    ...
    const botFatherLink = document.createElement("a");
    botFatherLink.href = "https://t.me/botfather";
    ...
    new Setting(containerEl)
      ...
      .addSearch((cb) => {
        ...
        cb.setPlaceholder('example: folder1/folder2')
          ...
          .onChange((newFolder) => {
            this.plugin.settings.newNotesLocation = newFolder;
    ...
    new Setting(containerEl)
      ...
      .addSearch((cb) => {
        ...
        cb.setPlaceholder('example: folder1/folder2')
          ...
          .onChange((newFolder) => {
            this.plugin.settings.newFilesLocation = newFolder;
            this.plugin.saveSettings();
          });
      });
    ...
    new Setting(containerEl)
      ...
      .setDesc('Template to use when creating new notes. Available variables: {{content}}, {{forwardFrom}}, {{messageDate:YYYYMMDD}}, {{messageTime:HHmmss}}.')
      .addSearch((cb) => {
        new FileSuggest(
          cb.inputEl,
          this.plugin
    ...
    new Setting(containerEl)
      ...
      .setDesc('All messages will be appended into a single file, Telegram.md. If disabled, a separate file will be created for each message.')
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.appendAllToTelegramMd)
    ...
    new Setting(containerEl)
      ...
      .setDesc('The Telegram messages will be deleted after processing them. If disabled, the Telegram messages will be marked as processed.')
      .addToggle((toggle) => {
...
const buyMeACoffeeButton = (link: string): HTMLElement => {
  ...
  a.innerHTML = `<img src="https://img.buymeacoffee.com/button-api/?text=Buy%20me%20a%20banana&emoji=🍌&slug=soberhacker&button_colour=FF5F5F&font_colour=ffffff&font_family=Cookie&outline_colour=000000&coffee_colour=FFDD00" height="42px"> `;
  return a;
};
...
const kofiButton = (link: string): HTMLElement => {
  const a = createEl("a");
  a.setAttribute("href", link);
  a.innerHTML = `<img src="https://storage.ko-fi.com/cdn/brandasset/logo_white_stroke.png?" width="145" height="47">`;
  return a;
};